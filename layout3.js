...
window.onload = function() {
	...
	//graph.addNode(0);
	ind = 0;
	graph.addNode(ind, null, "CLICK A NODE!", "link"+1, 0.3);//, 0);
	ind++;
	// for (var i=2; i<=15; i++) {
...
function addOne() {
	graph.addNode(ind, Math.floor(ind/2), "loooooooooooong text "+ind, "link"+ind, 0.3);
...
Graph.prototype.addNode = function(id, parent, text, link, width, height) {
	...
	this.n[id].left = Math.random();
	this.n[id].right = this.n[id].left;
	this.n[id].top = Math.random();
	...
	node.style.left = ((this.n[id].left-this.midx)*100-2.5).toString()+'%';
	node.style.width = ((this.n[id].right-this.n[id].left)*100+5).toString()+'%';
	node.style.top = ((this.n[id].top-this.midy)*100-2.5).toString()+'%';
	node.style.height = ((this.n[id].bottom-this.n[id].top)*100+5).toString()+'%';
...
Graph.prototype.step = function() {
	for (var key in this.n) {
		...
		for (var key2 in this.n[key].inSet) {
			...
			totWidth += this.n[n1].right - this.n[n1].left;
			totHeight += this.n[n1].bottom - this.n[n1].top;
		}
		var tall = (totHeight*1.2+0.01 > totWidth);	// tall or squarish
		...
		if (this.n[key].parent == null) {
			this.n[key].dl -= 0.001*(this.n[key].left-0.5);
			this.n[key].dr -= 0.001*(this.n[key].right-0.5);
			this.n[key].dt -= 0.001*(this.n[key].top-0.5);
			this.n[key].db -= 0.001*(this.n[key].bottom-0.5);
		}
	}
...
Graph.prototype.doPlacement = function() {
	if (this.autoFrame) {
		...
		for (var key in this.n) {
			minx = Math.min(minx, this.n[key].left);
			maxx = Math.max(maxx, this.n[key].right);
			miny = Math.min(miny, this.n[key].top);
			maxy = Math.max(maxy, this.n[key].bottom);
		}

		var dec = 0.9;

		var m = Math.max(maxx-minx, maxy-miny)+0.15;
		this.zoom = this.zoom*dec+(m ? 1/m : 1)*(1-dec);

		if (minx != Infinity) {
			this.midx = this.midx*dec+((minx+maxx)/2 - 0.5)*(1-dec);
			this.midy = this.midy*dec+((miny+maxy)/2 - 0.5)*(1-dec);
		}
	}
	...
	for (var key in this.n) {
		this.nodes[key].style.left = ((this.n[key].left-this.midx)*100-2.5).toString()+'%';
		this.nodes[key].style.width = ((this.n[key].right-this.n[key].left)*100+5).toString()+'%';
		this.nodes[key].style.top = ((this.n[key].top-this.midy)*100-2.5).toString()+'%';
		this.nodes[key].style.height = ((this.n[key].bottom-this.n[key].top)*100+5).toString()+'%';
	}
